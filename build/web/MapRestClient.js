...
// Model for Map entity
    models.Map = Backbone.Model.extend({
        ...
        sync: function (method, model, options) {
            ...
            if (method == 'create') {
                options.url = 'http://localhost:8084/CinemaWebApp/webresources/cinema.entities.map/';
            }
            if (method == 'update') {
                options.url = 'http://localhost:8084/CinemaWebApp/webresources/cinema.entities.map/';
            }
...
(function (views) {
    ...
    views.ListItemView = Backbone.View.extend({
        ...
        render: function (eventName) {
            ...
            // init datepicker
            this.$("#fromDate"+this.model.id).datepicker({
              showOn: "button",
              buttonImage: "img/calendar.jpg",
              buttonImageOnly: true
            });
            // add required to date
            this.$("#fromDate"+this.model.id).prop("required", true);
            return this;
        },
        close: function () {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.MapCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    ...
                    self.listView = new views.ListView({
                        ...
                        // tpl-map-list-itemis template identifier for item
                        templateName: '#tpl-map-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.map = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.map,
                    // tpl-map-details is template identifier for chosen model element
                    templateName: '#tpl-map-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Map();
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-map-details is a template identifier for chosen model element
                templateName: '#tpl-map-details',
